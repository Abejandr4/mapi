...
struct ContentView: View {
    ...
    func procesarIntencion(textoUsuario: String) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            SpeechManager.shared.speak(text: "Aquí está \(paisEncontrado.nombre). \(paisEncontrado.descripcionGeneral). Dato curioso: \(dato)")
        }
    }
    ...
    func abrirPais(_ pais: PaisInfo) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
            SpeechManager.shared.speak(text: "Aquí está \(pais.nombre). \(pais.descripcionGeneral). Dato curioso: \(dato)")
        }
    }
